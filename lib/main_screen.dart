...
class _MainScreenState extends State<MainScreen> {
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return ListTile(
            ...
            subtitle: Text('IMC: ${persons[index].bmi.toStringAsFixed(2)} - ${persons[index].thing}'),
            onTap: () {
  ...
  void _editPerson(BuildContext context, int index) async {
    ...
    if (result != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("${persons[index].name} editado(a) com sucesso!"),
        ),
      );
    }
  }
  ...
  void _deletePerson(int index) {
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("${persons[index].name} removido, adeus!"),)
      );
    setState(() {
